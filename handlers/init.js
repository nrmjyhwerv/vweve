...
async function init() {
    ...
    if (!skyport) {
        log.init('this is probably your first time starting Cryptalis, welcome!');
        log.init('you can find documentation for the panel at skyport.dev');

        let imageCheck = await db.get('images');
        if (!imageCheck) {
            log.error('before starting Cryptalis for the first time, you didn\'t run the seed command!');
        ...
        log.info('initialized Cryptalis panel with id: ' + skyportId)
    }        
